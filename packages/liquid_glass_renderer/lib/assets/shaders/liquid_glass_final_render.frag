...
void main() {
    ...
    if (edgeFactor > 0.01) {
        ...
        if (uGlassColor.r > 0.5) {
            // Dark recipe: keep the rim primarily neutral. The stock shader
            // amplifies backdrop hue at the perimeter, which creates a thin
            // blue/yellow outline as colored content crosses an edge.
            vec3 neutralHighlight = mix(vec3(bgLuminance), vec3(1.0), 0.65);
            highlightColor = mix(neutralHighlight, bgColor, 0.08);
            rimAmount = clamp(brightness * 0.32, 0.0, 0.30);
        } else {
            // Light recipe: retain the package's original rendering exactly.
            ...
            rimAmount = brightness;
        }

        finalColor.rgb = mix(finalColor.rgb, highlightColor, rimAmount);
    }
